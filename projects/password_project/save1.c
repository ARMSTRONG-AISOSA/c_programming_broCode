...
void takepassword(char pwd[50])
{
    ...
    while (1) // This loops indefinately until a break statement is stops it
    {
        ch = getch(); // Reads one character at a time using getch(), so input isn't visible to the user.

        if (ch == ENTER || ch == TAB) // ENTER (or TAB), it terminates the password string with \0 and breaks the loop (ending input).
        {
        ...
        { // Prevent buffer overflow (save space for null terminator)
            pwd[i++] = ch;
            printf("*");
            fflush(stdout);
        }
        // Ignore input if password is at maximum length
    }
